...
async def reklama_olish_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    text = "Reklama narxlarini ko'rish uchun: @anilordtvrek\n\nReklama olmoqchi bo'lsangiz, batafsil yozib yuboring:"
...
async def reklama_olish_receive(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text("✅ Xabaringiz adminga yuborildi.", reply_markup=get_main_menu_keyboard())
...
reklama_conv_handler = ConversationHandler(
    ...
    states={GET_REKLAMA: [MessageHandler(filters.TEXT & ~filters.COMMAND, reklama_olish_receive)]},
